...
test('Verify Smart Pricing Plan Calendar Month And Events For Homes', async ({ page }) => {
    ...
    if (currentUrl.includes('/about/delivering-energy/electric-projects/smart-meter-upgrades/smart-pricing-plan')) {
      ...
      await validateCalendarsForHome(page);
      //await validateCalendarsForBusiness(page);          
});

async function validateCalendarsForHome(page) {
    // Verifying calendar for Home is visible
    const calendarLocator = page.locator("(//div[@class='dom-calendar'])[1]");
    await calendarLocator.scrollIntoViewIfNeeded();
    await expect(calendarLocator).toBeVisible();
    console.log("Step 4: Classification Calendar For Homes is visible.");

    // Verifying calendar displays the current month and year
    const calendarDateLocator = page.locator("(//*[@class='dom-calendar'])[1]//*[@class='dom-calendar-row dom-calendar-title']//*[@class='dom-calendar-col dom-cal-col-5 dom-cal-date']");
    ...
    const totaldays= page.locator("(//*[@class='dom-calendar'])[1]//div[@class='dom-cal-day']");
     await page.waitForTimeout(500);
    ...
    const dayCells = page.locator("(//*[@class='dom-calendar'])[1]//div[contains(@class,'dom-calendar-col')]");
