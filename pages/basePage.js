...
class BasePage {
  ...
  async retryGoto(
    ...
    while (attempt < retries) {
      try {
        console.log(`Navigation attempt ${attempt + 1}/${retries}: ${url}`);
        ...
        // Wait for page DOM element
        await this.page.locator(selector).waitFor({
          state: "visible",
          timeout: 30000,
        });

        // Check for application error message
        const retryMessage = this.page.locator(
          'text="Close the browser and retry again"',
        );

        if (await retryMessage.isVisible().catch(() => false)) {
          console.log(
            'Detected in-page error: "Close the browser and retry again"',
          );
          ...
          if (attempt >= retries) {
            throw new Error(
              'Detected in-page error after maximum retries: "Close the browser and retry again"',
            );
          }
        ...
        console.log(
          `Retrying navigation (${attempt + 1}/${retries}) after ${delay}ms`,
        );
  ...
  async handleLocationVirginia() {
    ...
    if ((await locationModal.count()) && (await locationModal.isVisible())) {
      const virginiaOption = locationModal.locator(
  ...
  async handleLocationSouthCarolina() {
    ...
    if ((await locationModal.count()) && (await locationModal.isVisible())) {
      const virginiaOption = locationModal.locator(
        "a.tile.location-tile[data-location='South Carolina']",
      );
      await virginiaOption.click();
  ...
  async handleLocationNorthCarolina() {
    ...
    if ((await locationModal.count()) && (await locationModal.isVisible())) {
      const virginiaOption = locationModal.locator(
        "a.tile.location-tile[data-location='North Carolina']",
      );
      await virginiaOption.click();
  ...
  async handleChooselater() {
    ...
    if (await modal.isVisible()) {
      const chooseLater = this.page.locator(
        "//a[@class='cta-btns-text' and contains(text(), 'Choose Later')]",
      );
  ...
  async checkNetworkStatus() {
    ...
    this.page.on("response", (response) => {
      ...
      if (response.url().startsWith(mainPageUrl) && !response.ok()) {
        console.log(
          `Response error: ${response.status()} for ${response.url()}`,
        );
        networkError = true;
      }
    });
